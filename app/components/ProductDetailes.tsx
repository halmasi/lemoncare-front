...
export default function ProductDetailes({
  ...
  const getDetailesFn = useMutation({
    mutationFn: async () => {
      const res = Promise.all(
        product.detailesTable.map(async (item) => {
          if (item && item.detaile_key && item.detaile_value) {
    ...
    onSuccess: (data) => {
      if (!data) return;
      setDetaileTable(
        data.filter((item): item is { key: string; value: string } => !!item)
      );
    },
  });
    ...
    <>
      <div className="mt-5 md:mx-10 bg-slate-50 border rounded-lg">
        <div className="flex flex-col items p-2">
          ...
          <div className={`${showContent ? 'h-fit' : 'h-32'} overflow-hidden`}>
      ...
      {getDetailesFn.isPending ? (
        <LoadingAnimation />
      ) : (
        detaileTable &&
        detaileTable.length > 0 && (
          <div className="mt-5 md:mx-10 bg-slate-50 border rounded-lg">
            <div className="flex flex-col items p-2">
